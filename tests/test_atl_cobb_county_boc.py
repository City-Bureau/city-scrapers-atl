...
def test_title():
    assert (
        parsed_items[0]["title"] == "Board of Commissioners Agenda Work Session"
    )  # noqa
...
def test_id():
    assert (
        parsed_items[0]["id"]
        == "atl_cobb_county_boc/202404220900/x/board_of_commissioners_agenda_work_session"  # noqa
    )
...
def test_links():
    expected_links = [
        {
            "href": "https://www.cobbcounty.org/events/23143",
            "title": "Event Details",
        }  # noqa
    ]
...
def test_source():
    assert (
        ...
        == "https://www.cobbcounty.org/events?field_event_type_target_id=326"
    )
